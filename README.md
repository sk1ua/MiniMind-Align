...
本仓库的 MiniMind-Align 扩展位于 `docs/`、`align/`、`trainer/`、`evaluation/` 和 `demo/`。
它在 MiniMind 基线上记录了 Alignment v2 数据工程、SFT/LoRA、on-policy DPO/SimPO、轻量 Reward Model 以及规则奖励 GRPO/CISPO lite pilot。

当前交付基于 Google Cloud NVIDIA L4 24GB，使用独立 experiment id、权重 SHA256、冻结 100 条规则评测和失败目录留痕。
结果显示 SimPO 64-step pilot 是当前冻结规则集的 validator leader；256-step full 由于长度/重复坍缩被保留为失败分析对照，不作为默认模型。
补做的完整 Gemini 冻结集对照中，DPO v2 对 align_sft_v2 为 `tie 93 / DPO 6 / SFT 1`，SimPO pilot 为 `tie 77 / SimPO 16 / SFT 7`；这支持“SimPO 有小幅主观增益、DPO 增量很小”的谨慎结论。
SimPO full 的 Gemini 对照为 `tie 47 / SimPO full 22 / SFT 31`，平均 overall 低于 align_sft_v2，进一步支持将其作为长度/重复过度优化的负结果。
GRPO/CISPO 只完成 tiny pilot，不能据此宣称收敛或算法优劣；账单美元金额没有在本机伪造，成本报告使用可复核的 wrapper wall time 代理。

- 实验总报告：`docs/experiment_report.md`
- 失败分析：`docs/failure_analysis.md`
- 成本与资源：`docs/cost_report.md`
- 复现说明：`docs/reproduction.md`
- 五分钟介绍与面试问答：`docs/five_minute_pitch.md`、`docs/interview_qa.md`
- 路线图与任务状态：`docs/MINIMIND_ALIGN_ROADMAP.md`、`docs/TASKS.md`
- Streamlit Demo：`demo/app.py`

默认只做无训练 dry-run；确认资源后再执行：
