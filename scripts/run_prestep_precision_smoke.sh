...
MODE="${1:---dry-run}"
ROOT="${PRESTEP_PRECISION_ROOT:-results/experiments/rl_prestep_precision_smoke_20260802}"
...
SESSION_NAME="${PRESTEP_PRECISION_TMUX_SESSION:-rl_prestep_precision_smoke_20260802}"
HARD_LIMIT_SECONDS="${PRESTEP_PRECISION_HARD_LIMIT_SECONDS:-1800}"
TASK_ID="${PRESTEP_PRECISION_TASK_ID:-MM-E023}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  echo "pre-step training-loss precision smoke dry run"
  echo "experiment root: $ROOT"
  echo "production loss: unchanged bfloat16 autocast; shadow: fp32 no-autocast"
...
payload = {
    ...
    "pre_step_precision": {
        "production_loss": "bfloat16_autocast_unchanged",
        "shadow_loss": "fp32_no_autocast",
...
RUN_DIR="$ROOT/grpo_prestep_precision_smoke_seed42"
if [[ -e "$RUN_DIR" ]]; then
...
CMD=(.venv/bin/python trainer/train_grpo_lite.py
  ...
  --interleave-categories --save-dir "$RUN_DIR" --save-weight grpo_prestep_precision_smoke_seed42)
...
(
  ...
  echo "ENV_HASH=$(sha256sum trainer/train_grpo_lite.py align/rl_rules.py evaluation/audit_prestep_precision.py scripts/run_prestep_precision_smoke.sh "$TRAIN_MANIFEST" "$VALIDATION_MANIFEST" | sha256sum | cut -d' ' -f1)"
...
if [[ -f "$RUN_DIR/selection.json" ]]; then
  PYTHONPATH=. .venv/bin/python evaluation/audit_prestep_precision.py \
    --experiment-root "$ROOT" --output-dir "$ROOT/prestep_precision_audit" \
