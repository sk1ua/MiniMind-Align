...
MODE="${1:---dry-run}"
ROOT="${NATURAL_RULE_REWARD_ROOT:-results/experiments/rl_natural_rule_reward_smoke_20260802}"
...
SESSION_NAME="${NATURAL_RULE_REWARD_TMUX_SESSION:-rl_natural_rule_reward_20260802}"
HARD_LIMIT_SECONDS="${NATURAL_RULE_REWARD_HARD_LIMIT_SECONDS:-1800}"
TASK_ID="${NATURAL_RULE_REWARD_TASK_ID:-MM-E027}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  echo "natural rule-reward corrected active-path smoke dry run"
  echo "experiment root: $ROOT"
  echo "reward source: rule_reward; controlled override: disabled"
...
output.write_text(json.dumps({
    ...
    "validation_count": validation_count, "reward_source": "rule_reward",
...
RUN_DIR="$ROOT/grpo_natural_rule_reward_smoke_seed42"
if [[ -e "$RUN_DIR" ]]; then
...
CMD=(.venv/bin/python trainer/train_grpo_lite.py
  ...
  --interleave-categories --save-dir "$RUN_DIR" --save-weight grpo_natural_rule_reward_smoke_seed42)

(
  echo "STARTED_AT=$(date -Is)"
  echo "MODE=--smoke TASK_ID=$TASK_ID"
  echo "COMMAND=$(printf '%q ' "${CMD[@]}")"
  echo "REWARD_SOURCE=rule_reward CONTROLLED_REWARD_OVERRIDE=false"
  echo "GIT_COMMIT=$(git rev-parse HEAD)"
  echo "ENV_HASH=$(sha256sum trainer/train_grpo_lite.py align/rl_rules.py evaluation/audit_natural_rule_reward_smoke.py scripts/run_natural_rule_reward_smoke.sh "$TRAIN_MANIFEST" "$VALIDATION_MANIFEST" | sha256sum | cut -d' ' -f1)"
...
if [[ -f "$RUN_DIR/selection.json" ]]; then
  PYTHONPATH=. .venv/bin/python evaluation/audit_natural_rule_reward_smoke.py \
    --experiment-root "$ROOT" --output-dir "$ROOT/natural_rule_reward_audit" \
    --run-name "$(basename "$RUN_DIR")" --expected-training-forward-mode fp32_no_autocast \
    --expected-steps 2 || AUDIT_STATUS=$?
else
