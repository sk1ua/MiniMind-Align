...
MODE="${1:---dry-run}"
ROOT="${FP32_TRAINING_FORWARD_ROOT:-results/experiments/rl_fp32_training_forward_smoke_20260802}"
...
SESSION_NAME="${FP32_TRAINING_FORWARD_TMUX_SESSION:-rl_fp32_training_forward_smoke_20260802}"
HARD_LIMIT_SECONDS="${FP32_TRAINING_FORWARD_HARD_LIMIT_SECONDS:-1800}"
TASK_ID="${FP32_TRAINING_FORWARD_TASK_ID:-MM-E024}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  echo "fp32 training-forward precision smoke dry run"
  echo "experiment root: $ROOT"
  echo "active loss: fp32 no-autocast; legacy bfloat16 remains a shadow"
...
payload = {
    ...
    "pre_step_precision": {
        "production_loss": "fp32_no_autocast_opt_in",
        "legacy_shadow_loss": "bfloat16_autocast",
        "active_training_forward_mode": "fp32_no_autocast",
        "same_rollout_mask_old_ref_advantages": True,
        "shadow_gradient": "torch.autograd.grad_without_parameter_grad_mutation",
        "token_replay": True,
    },
...
RUN_DIR="$ROOT/grpo_fp32_training_forward_smoke_seed42"
if [[ -e "$RUN_DIR" ]]; then
...
CMD=(.venv/bin/python trainer/train_grpo_lite.py
  ...
  --interleave-categories --save-dir "$RUN_DIR" --save-weight grpo_fp32_training_forward_smoke_seed42)
...
(
  ...
  echo "ENV_HASH=$(sha256sum trainer/train_grpo_lite.py align/rl_rules.py evaluation/audit_fp32_training_forward.py scripts/run_fp32_training_forward_smoke.sh "$TRAIN_MANIFEST" "$VALIDATION_MANIFEST" | sha256sum | cut -d' ' -f1)"
...
if [[ -f "$RUN_DIR/selection.json" ]]; then
  PYTHONPATH=. .venv/bin/python evaluation/audit_fp32_training_forward.py \
    --experiment-root "$ROOT" --output-dir "$ROOT/fp32_training_forward_audit" \
    --run-name "$(basename "$RUN_DIR")" --expected-training-forward-mode fp32_no_autocast || AUDIT_STATUS=$?
else
  echo "selection.json missing; preserving run logs without audit" >&2
  AUDIT_STATUS=2
fi
echo "PRESTEP_PRECISION_GPU_WALL_SECONDS=$(( $(date +%s) - START_EPOCH ))"
