...
MODE="${1:---dry-run}"
ROOT="${KL_REPLAY_SMOKE_ROOT:-results/experiments/rl_corrected_kl_replay_smoke_20260802}"
...
SESSION_NAME="${KL_REPLAY_SMOKE_TMUX_SESSION:-rl_corrected_kl_replay_smoke_20260802}"
HARD_LIMIT_SECONDS="${KL_REPLAY_SMOKE_HARD_LIMIT_SECONDS:-1800}"
TASK_ID="${KL_REPLAY_SMOKE_TASK_ID:-MM-E021}"
...
if [[ "$MODE" == "--dry-run" ]]; then
  echo "corrected KL token-replay smoke dry run"
  echo "experiment root: $ROOT"
  echo "mode: GRPO seed=42; BF16 production gate plus no-autocast/full-FP32 replay"
...
payload = {
    ...
    "guard": {
        ...
        "backoff_factor": 0.5,
        "max_backoffs": 3,
        "production_measurement": "bfloat16_autocast",
        "corrected_measurements": ["bfloat16_no_autocast", "full_float32_no_autocast"],
        "token_replay": True,
...
RUN_DIR="$ROOT/grpo_corrected_kl_replay_smoke_seed42"
if [[ -e "$RUN_DIR" ]]; then
...
CMD=(.venv/bin/python trainer/train_grpo_lite.py
  ...
  --interleave-categories --save-dir "$RUN_DIR" --save-weight grpo_corrected_kl_replay_smoke_seed42)
...
(
  ...
  echo "ENV_HASH=$(sha256sum trainer/train_grpo_lite.py align/rl_rules.py evaluation/audit_kl_token_replay.py scripts/run_corrected_kl_replay_smoke.sh "$TRAIN_MANIFEST" "$VALIDATION_MANIFEST" | sha256sum | cut -d' ' -f1)"
...
echo "KL_REPLAY_SMOKE_GPU_WALL_SECONDS=$(( $(date +%s) - START_EPOCH ))"
exit "$RUN_STATUS"
