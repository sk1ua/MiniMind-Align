...
if __name__ == "__main__":
    ...
    parser.add_argument("--tokenizer_path", type=str, default="model", help="tokenizer目录，按当前工作目录解析")
    parser.add_argument("--model_dir", type=str, default="out", help="初始化权重目录，按当前工作目录解析")
    ...
    parser.add_argument("--max_steps", type=int, default=0, help="每个运行最多执行的 batch step；0 表示完整 epoch")
    ...
    model, tokenizer = init_model(lm_config, args.from_weight, tokenizer_path=args.tokenizer_path, save_dir=args.model_dir, device=args.device)
    attached_modules = apply_lora(model, rank=args.lora_rank, alpha=args.lora_alpha, dropout=args.lora_dropout)
    Logger(f"LoRA target modules: {len(attached_modules)} ({', '.join(attached_modules[:4])})")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0: 
            ...
            train_epoch(epoch, loader, effective_iters + skip, lora_params, start_step, wandb)
        else:
            train_epoch(epoch, loader, effective_iters, lora_params, 0, wandb)
